---
title: "Introduction to R key, STAMPS 2025"
...
```{r, eval = FALSE}
clr_with_pseudo <- function(counts, pseudo = 0.5) { # add optional argument
  new_counts <- counts
...
1. Read the "phylum_data" file in the "data" folder and create a new data frame in which you make a new column for the sum of counts of *Acidobacteria* and *Fusobacteria*, maintain only the variables "Location", "Month", "Season", "Type" and the new column you created, and retain only samples that have positive counts in the new column. 

```{r, eval = FALSE}
phylum_data <- read.csv("data/phylum_data.txt", sep = "\t", row.names = 1, header = T)

new_phylum_data <- phylum_data %>% mutate(acid_fuso = Acidobacteria + Fusobacteria) %>%
  filter(acid_fuso > 0) %>% 
  select(Location, Month, Season, Type, acid_fuso)
```
...
```{r}
agg_data <- phylum_data %>%
  group_by(Location, Season) %>%
  summarise(prop_tm6 = mean(TM6 > 0),
            prop_tm7 = mean(TM7 > 0))
