...
Since in this model we are using a log link function, our coefficients describe how the log(expected counts) differs between saliva samples and sputum samples. A p-value of 0.01 (at an alpha level of 0.05) indicates we have enough evidence to reject there is no difference between these log(expected counts). And by exponentiating the estimated coefficient (exp(0.73) = 2.08), we can say that the estimated expected counts observed for samples of Sputum is 2.08 times higher than the estimated expected counts for samples of saliva.
