...
Note that by fitting a linear model, we are targeting a parameter that represents differences in means. We could also choose to target a parameter that represents fold-differences in means. We'll see an example of this at the end of the lab. Both options are valid, as long as you correctly interpret the coefficients in terms of the model that you've chosen. This decision is driven by which parameter is more relevant to your scientific question.    
...
We have 4 columns. The first column gives the estimate of the coefficient for the intercept or covariate within the row. The second column gives the standard error (remember, this is an estimate of the standard deviation of our estimate). 

By default, `lm` will run hypothesis tests for each regression parameter in the model. In a linear model, this means that for each covariate (or covariate level), it will test the hypothesis that the expected difference in the outcome associated with a one unit increase in that covariate, when holding all other covariates equal, is equal to 0. The third column of the coefficient table output gives a test statistic for this hypothesis, and the fourth column provides a p-value.  
...
```{r}
mod_interact <- lm(ddpcr ~ `Sample Type` + `Treatment Group` + 
                     `Sample Type` : `Treatment Group`, both) # Corrected typos and orders of arguments
...
If you were running this analysis, you could decide how complex you wanted your model to be, and whether or not you wanted to include antibiotic use. 
...
Now, the results are a little different. The argument `effect` tells us the type of effect we're looking for, `total` means that we care about the effect of disease on microbe abundance through both direct causal pathways and indirect pathways (including the path from disease to antibiotic use to microbe abundance). Typically, unless otherwise specified, we ask questions about the total effect. When we want the total effect, we should avoid including antibiotic use in our adjustment set, because if we account for it separately then we won't be able to include it in our disease effect. 

However, if we want the direct effect, i.e. only the effect of disease on microbe abundance through the direct path, not the indirect effect through antibiotic use, then we can use `effect = "direct"`. This adjustment set does include age, diet, and antibiotic use. 

### Scenario 3 

The growth rate of bacteria can be impacted by temperature. You observe that the transcript abundance of the common cell division ftsZ gene changes based on ocean temperature, and want to study this effect. You also observe that transcript abundance of the ftsZ gene is affected by time of day. Additionally, both ocean temperature and the abundance of organisms carrying the ftsZ gene are affected by season. Ocean temperature also affects nutrient concentrations, which in turn affects the abundance of organisms carrying gene A. 
...
```{r}
dag3 <- dagify(
  abundance ~ temp + day + season + nutrient,
  temp ~ season,
  nutrient ~ temp,
  exposure = "temp",
  outcome = "abundance"
...
ggdag(dag3, text = FALSE) +
  ...
  geom_dag_label(size = 3, alpha = 0.7)
# get adjustment sets
adjustmentSets(dag3, type = "minimal", effect = "total")
adjustmentSets(dag3, type = "canonical", effect = "total")
adjustmentSets(dag3, type = "minimal", effect = "direct")
```
