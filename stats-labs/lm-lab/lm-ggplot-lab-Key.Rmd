...
`lm` will by default run a hypothesis test with the hypothesis that the coefficient for each covariate is equal to 0. If this hypothesis is rejected, we determine that there is statistically significant relationship between our covariate and outcome (after accounting for other covariates). The third column of our output gives the test statistic for that hypothesis test, and the fourth column gives a p-value. 
