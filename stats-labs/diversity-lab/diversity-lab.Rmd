...
Now, we're particularly interested in looking at only the water samples so we are going to need to subset our samples using the variable SampleType. You can use the `subset_samples()` function from `phyloseq`. 

```{r, eval=FALSE}
water <- GlobalPatterns %>%
  _________(SampleType %in% c("Freshwater",
                                   "Freshwater (creek)",
                                   "Ocean",
                                   "Sediment (estuary)"))
```
...
Let's say we're interested in understanding the diversity in our water samples at the Order level. You can use the `tax_glom()` function from `phyloseq` to summarize our data at the Order level.

```{r, eval=FALSE}
water_order <- water %>% __________("Order")
```
...
Fill in the appropriate blanks for the plot aesthetics below to plot how observed richness differs across the depth of samples that are colored by sample type.
(Hint: If we want to see how observed richness changes as sequencing depth changes, think about how we could set the axes based on the plot we made previously plotting how richness changes across samples!)
...
data.frame("observed_richness" = (observed_phyloseq %>% summary)$estimate,
           ...
           "type" = water_order %>% sample_data %>% subset_samples("SampleType")) %>%
  ggplot(aes(x = __________, y = _____________, color = _______________)) +
  geom_point() + 
...
Fill in the blanks below appropriates so we can first store a phyloseq object for just the 
TRRsed1 sample.

```{r, eval = FALSE}
tr <- water_order %>% subset_samples(X.SampleID == ______________)
