...
there is also an important optional argument (optional when `run_score_tests = FALSE`, which we use for now):
...
Here we can see that the minimum q-value from `radEmu` is $0.12$, the minimum q-value from `ALDEx2` is $0.27$, the minimum q-value from `ANCOM-BC2` is $8.96e-8$ and the minimum q-value from `DESeq2` is $7.24e-11$. 
...
That said, in some StatDivLab simulations with large sample sizes, `ANCOM-BC2`, `DESeq2`, and `LinDA` fail to control Type I error, which is why the StatDivLab would hesitate to claim "significant! FDR<0.1!" here. `radEmu` and `ALDEx2` do control Type I error in our simulation settings, and don't find anything significant at the 10% FDR level here... there's just not enough strength of evidence at this stringency threshold...  
...
- `ALDEx2`
  ...
  - cons: is pretty conservative (lower power), uses a pseudocount internally
- `ANCOM-BC2`
  - pros: high power, faster than `radEmu` to test all categories
  - cons: fails to control Type I error rate in some situations, can't handle data separation (a common consequence of sparsity, taxa that appear only in samples from one covariate level and not another)
- `DESeq2`
  - pros: faster than `radEmu` to test all categories 
  - cons: fails to control Type I error in some situations, tailored for RNA-seq data not microbiome HTS data
- `LinDA`
  - pros: faster than `radEmu` to test all categories, high power in some settings
  - cons: fails to control Type I error in some settings (large sample sizes with sparse data), uses a pseudocount internally
