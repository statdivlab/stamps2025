...
We estimate that the log fold-difference in expected abundance in cases compared to controls in *Fusobacterium nucleatum s. vincentii [ref_mOTU_v2_0754]* compared to the log fold-difference in expected abundance in cases compared to controls in *Streptococcus salivarius [ref_mOTU_v2_0199] * is $1.29$.

You could also interpret this in terms of fold-differences, using `exp(1.29) = 3.63`. 
...
```{r}
ch_fit$coef$estimate[1] - ch_fit_ref$coef$estimate[1]
# this is equal to the estimate of Streptococcus salivarius [ref_mOTU_v2_0199] in ch_fit
ch_fit$coef$estimate[ref_num]
```

The difference is $-0.24$. All of the estimates are shifted by this value, because we have changed the parameter that we target when we choose a reference taxon. 
...
This will depend on what mOTU you chose! 
...
You may also notice that ALDEx2 has a peak towards 0.75, meaning that the majority of p-values are pretty large, and that there are few ALDEx2 p-values greater than 0.8. You may also notice that ANCOM-BC2 p-values are bimodal (peaks around 0 and 1), as are DESeq2 p-values, or that LinDA p-values peak around 0.05. 
...
You may also notice that ALDEx2 and LinDA p-values have the strongest association. You may notice that there is a concentration of mOTUs with large ALDEx2 p-values and small radEmu p-values. 
...
A q-value of $0.07$ means that if all mOTUs with q-values of $0.07$ or smaller were considered to be discoveries, we would expect $7\%$ of fewer of them to be false discoveries. 
...
There are many good answers to this, and they partially depend on your scientific background, and if any of these taxa and their association with increased or decreased abundance in colorectal cases or controls are particularly interesting to you. 

I would like to know more about *unknown Prevotella [meta_mOTU_v2_6294]*, which has a small q-value and is estimated to have a `exp(5.95) = 384`-fold decrease in abundance in cases compared to controls. 
