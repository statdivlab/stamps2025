...
sum(rowSums(small_Y) == 0) # zero samples have a count sum of 0
...
<!-- (as opposed to controls) minus the *typical* log fold difference associated with cases versus controls across all $46$ mOTUs in this analysis is $1.486$.  -->

By default `radEmu` estimates this type of parameter, in which we compare the log fold difference in one mOTU to the *typical* (approximately the median) log fold difference in all mOTUs. However, `radEmu` is flexible -- you can estimate other types of parameters. For example, we can instead compare log fold differences to the log fold difference for a specific (or reference) mOTU. 
...
- `DESeq2`
  ...
  - cons: fails to control Type I error in some situations, tailored for RNA-seq data, not microbiome data
- `LinDA`
  - pros: faster than `radEmu` to test all categories, high power in some settings
  - cons: fails to control Type I error in some situations (large sample sizes with sparse data), uses a pseudocount 
